...
class AdminDashboard {
    ...
    async loadStats() {
        try {
            console.log('📊 Loading stats via True Hybrid Storage...');
            
            // GUNAKAN HYBRID STORAGE SECARA KONSISTEN
            const statistik = await hybridStorage.getStatistikFromFirebase();
            this.updateStatsUI(statistik);
            
        } catch (error) {
            console.error('Error loading stats via Hybrid:', error);
    ...
    async loadStatsFallback() {
        try {
            console.log('🔄 Loading stats via localStorage fallback...');
    ...
    async loadRecentPendaftaran() {
        try {
            console.log('👥 Loading recent participants via Hybrid Storage...');
            
            // GUNAKAN HYBRID STORAGE
            const peserta = await hybridStorage.getAllPesertaFromFirebase();
            ...
            await this.renderRecentPendaftaran(container, recentPeserta);
            
        } catch (error) {
            console.error('Error loading recent pendaftaran via Hybrid:', error);
            await this.loadRecentPendaftaranFallback();
        }
    }

    async loadRecentPendaftaranFallback() {
        try {
            ...
            console.error('Recent pendaftaran fallback failed:', error);
        }
    }
    ...
    async loadPaketPerformance() {
        try {
            console.log('📈 Loading package performance via Hybrid Storage...');
            ...
            console.error('Error loading package performance via Hybrid:', error);
            await this.loadPaketPerformanceFallback();
        }
    }

    async loadPaketPerformanceFallback() {
        try {
            ...
            console.error('Package performance fallback failed:', error);
        }
    }
