...
class AdminPaket {
    ...
    // Load paket data table - HYBRID VERSION
    async loadPaketTable() {
        try {
            console.log('📦 Loading packages via True Hybrid Storage...');
            
            // GUNAKAN HYBRID STORAGE SECARA KONSISTEN
            const [paket, peserta] = await Promise.all([
                hybridStorage.getAllPaketFromFirebase(),
                hybridStorage.getAllPesertaFromFirebase()
            ]);
            
            console.log(`✅ Hybrid Data: ${paket.length} packages, ${peserta.length} participants`);
            this.renderPaketTable(paket, peserta);
            
        } catch (error) {
            console.error('Error loading packages via Hybrid:', error);
            await this.loadPaketTableFallback();
        }
    }

    // Fallback hanya jika hybrid benar-benar gagal
    async loadPaketTableFallback() {
        try {
            ...
            this.showTableError('Sistem tidak dapat memuat data paket.');
        }
    }
    ...
    showTableError(message) {
        ...
        emptyState.innerHTML = `
            <div class="text-center py-12">
                ...
                <button onclick="window.adminPaketInstance.loadPaketTable()" class="mt-4 bg-blue-900 hover:bg-blue-800 text-white font-semibold py-2 px-4 rounded-xl transition duration-200">
    ...
    async savePaket() {
        ...
        try {
            console.log('💾 Saving package via Hybrid Storage...');
            
            // GUNAKAN HYBRID STORAGE SECARA KONSISTEN
            // Jika edit, tambahkan ID
            if (isEdit && paketId) {
                paketData.id = paketId;
            }
            
            const result = await hybridStorage.savePaket(paketData, true); // true = isAdmin
            
            if (result) {
                this.showAlert('success', isEdit ? 'Paket berhasil diperbarui' : 'Paket baru berhasil ditambahkan');
                await this.loadPaketTable(); // Reload dengan hybrid
            ...
            console.error('Error saving package via Hybrid:', error);
            this.showAlert('error', 'Gagal menyimpan paket: ' + error.message);
        }
    }

    // Show delete confirmation modal
    async showDeleteModal(paketId) {
        try {
            // GUNAKAN HYBRID STORAGE untuk mendapatkan data
            const [paket, peserta] = await Promise.all([
                hybridStorage.getPaketById(paketId),
                hybridStorage.getAllPesertaFromFirebase()
            ]);
            
            if (!paket) {
                this.showAlert('error', 'Paket tidak ditemukan');
                return;
            }

            const pesertaCount = peserta.filter(p => p.paketId === paketId).length;
            ...
            console.error('Error loading delete confirmation data:', error);
            this.showAlert('error', 'Gagal memuat data konfirmasi hapus');
        }
    }

    // Confirm delete paket - HYBRID VERSION
    async confirmDelete() {
        ...
        try {
            console.log('🗑️ Deleting package via Hybrid Storage...');
            
            // GUNAKAN HYBRID STORAGE untuk delete
            const success = await this.deletePaketFromHybrid(this.paketToDelete);
            
            if (success) {
                // Reload table dengan hybrid
                await this.loadPaketTable();
                ...
                throw new Error('Gagal menghapus paket dari sistem');
            }
            
        } catch (error) {
            console.error('Error deleting package:', error);
            this.showAlert('error', 'Gagal menghapus paket');
        }
    }

    // Delete paket dari hybrid storage
    async deletePaketFromHybrid(paketId) {
        try {
            // Untuk hybrid storage, kita hapus dari Firebase dan sync ke localStorage
            if (window.hybridStorage && hybridStorage.firebaseReady) {
                await firebaseApp.db.collection('packages').doc(paketId).delete();
                console.log('✅ Package deleted from Firebase');
            }
            
            // Selalu sync ke localStorage
            const localPaket = storage.getPaket();
            const updatedPaket = localPaket.filter(p => p.id !== paketId);
            storage.setPaket(updatedPaket);
            console.log('✅ Package deleted from localStorage');
            
            return true;
        } catch (error) {
            console.error('Error deleting package from Hybrid:', error);
            return false;
        }
    }
