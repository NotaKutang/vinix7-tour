...
function getAdminWhatsAppNumber() {
    try {
        ...
        // Ambil nomor dari contact_phone, hapus karakter non-digit
        const phoneNumber = settings.contact_phone || '+62 812-3456-7890';
        return phoneNumber.replace(/\D/g, ''); // Hapus semua non-digit
    } catch (error) {
        console.error('Error getting admin WhatsApp number:', error);
        return '6281234567890'; // Fallback default
    }
}
...
async function cekStatus() {
    ...
    statusResult.innerHTML = `
        <div class="text-center py-8">
            ...
            <p class="text-gray-600">Mencari data peserta...</p>
        </div>
    `;
    statusResult.classList.remove('hidden');
    
    try {
        // Find peserta menggunakan hybrid storage
        console.log('🔍 Searching participant with code:', kodePendaftaran);
...
function showStatusResult(peserta, fromRecovery = false) {
    ...
    switch(peserta.status) {
        ...
        case 'verified':
            ...
            actionButton = `
                <a href="${peserta.linkGrup || getWhatsAppTemplate()}" target="_blank" class="btn-primary w-full md:w-auto py-3 px-6 text-center">
...
function hubungiAdmin() {
    const phoneNumber = getAdminWhatsAppNumber();
